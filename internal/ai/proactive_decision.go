...
func (dc *DecisionContext) ShouldPromptAI() bool {
	...
	// 最近 60 分钟内有消息时不触发，或时间未知时不触发
	if dc.MinutesSinceLast < 0 || dc.MinutesSinceLast < 60 {
		return false
	}
...
2. 如果活动水平为"low"或"medium"，且距离最后消息超过 60 分钟，可以考虑发送消息
3. 如果今日主动消息数 >= 5，不应该发送消息（避免骚扰）
4. 如果距离最后消息时间 < 60 分钟，不应该发送消息（避免干扰）
