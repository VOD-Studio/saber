...
// NewGIFCommand 创建一个新的 GIF 命令处理器。
func NewGIFCommand(sender Sender, text TextOnlySender, svc MemeService) *GIFCommand {
	return &GIFCommand{
		sender: sender,
		text:   text,
		svc:    svc,
...
// Handle 实现 CommandHandler。
func (c *GIFCommand) Handle(ctx context.Context, userID id.UserID, roomID id.RoomID, args []string) error {
	if c.svc == nil || !c.svc.IsEnabled() {
		return c.text.SendText(ctx, roomID, "Meme 服务未启用")
	}

	if len(args) == 0 {
		return c.text.SendText(ctx, roomID, "请提供搜索关键词，例如：!gif happy")
	}

	return c.text.SendText(ctx, roomID, fmt.Sprintf("搜索 GIF：%v", args))
}

// StickerCommand 处理 !sticker 命令。
...
// NewStickerCommand 创建一个新的 Sticker 命令处理器。
func NewStickerCommand(sender Sender, text TextOnlySender, svc MemeService) *StickerCommand {
...
// Handle 实现 CommandHandler。
func (c *StickerCommand) Handle(ctx context.Context, userID id.UserID, roomID id.RoomID, args []string) error {
	if c.svc == nil || !c.svc.IsEnabled() {
		return c.text.SendText(ctx, roomID, "Meme 服务未启用")
	}

	if len(args) == 0 {
		return c.text.SendText(ctx, roomID, "请提供搜索关键词，例如：!sticker hello")
	}

	return c.text.SendText(ctx, roomID, fmt.Sprintf("搜索 Sticker：%v", args))
}
