...
// TypedMemeCommand 是指定定内容类型的 meme 命令。
// 用于 !gif 和 !sticker 便捷命令。
type TypedMemeCommand struct {
	service     *Service
	cmdService  CommandService
	client      *mautrix.Client
	uploadMedia UploadMediaFunc
...
// NewTypedMemeCommand 创建一个指定内容类型的 meme 命令。
func NewTypedMemeCommand(cmdService CommandService, client *mautrix.Client, memeService *Service, contentType ContentType) *TypedMemeCommand {
	return &TypedMemeCommand{
		service:     memeService,
		cmdService:  cmdService,
		client:      client,
		uploadMedia: defaultUploadMedia(client),
...
// Handle 实现 CommandHandler。
func (c *TypedMemeCommand) Handle(ctx context.Context, userID id.UserID, roomID id.RoomID, args []string) error {
	// 检查服务是否可用
	if c.service == nil || !c.service.IsEnabled() {
		return c.cmdService.SendText(ctx, roomID, "Meme 服务未启用，请在配置中设置 meme.enabled=true 和 meme.api_key")
	}
	...
	if query == "" {
		typeNames := map[ContentType]string{
			ContentTypeGIF:     "GIF",
			ContentTypeSticker: "Sticker",
			ContentTypeMeme:    "Meme",
		}
		return c.cmdService.SendText(ctx, roomID, fmt.Sprintf("请提供搜索关键词，例如：!%s happy", strings.ToLower(typeNames[c.contentType])))
	}

	slog.Info("处理类型化 meme 命令",
		"user", userID.String(),
		"room", roomID.String(),
		"query", query,
		"type", c.contentType)
	...
	// 搜索 GIF
	gif, err := c.service.GetRandom(ctx, query, c.contentType)
	if err != nil {
		slog.Error("搜索失败", "error", err, "query", query)
		return c.cmdService.SendText(ctx, roomID, fmt.Sprintf("搜索「%s」失败：%v", query, err))
	}
