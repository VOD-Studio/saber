...
- Violet 回复在任务受理时创建空正文 `pending` 消息，随后以同一幂等键和消息 ID 更新思考摘要、正文、生成状态与失败原因；生成期间每 15 秒续期，终态投递保留失败前已生成的正文
...
- `web_fetch` 在 DNS 解析前拒绝十进制、十六进制、八进制及缩写形式的 IPv4 主机名，避免 SSRF 防护结果随系统解析器变化
