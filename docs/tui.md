...
| 刷新连接与会话 | F5 |
| 帮助 | `/help` 或 `/` |
| 停止当前任务 | Ctrl+C；会话保留，可继续发消息 |
| 退出界面 | Ctrl+Q / `/quit`；空闲且输入为空时 Ctrl+C 也可退出 |

模型和思考等级选择只影响后续消息，不改写配置，也不改变已运行任务。可用思考等级由上游模型决定；选择器提供常见值，不代表所有模型都支持。状态栏显示当前生效值。

宽窗口显示会话侧栏，小于 108 列时收起；历史选择始终可以通过 Ctrl+O 打开。回答区支持 Markdown、代码高亮、工具状态、用量和耗时。工具内容默认折叠；展开时界面最多显示每项前 4000 字符，持久化记录保留完整的受限工具输出。窗口至少需要 36×14。
...
- TUI 订阅持久化事件，自动携带游标重连。退出、切换会话、关闭终端均不取消任务；Ctrl+C 才发送取消请求。
- Matrix 单独注册终态投递器，保留原有重试和文件投递。没有订阅者或 Matrix 投递失败，不会重新执行任务。
- 服务重启将原先运行中的任务标记为中断，不盲目重放有副作用的工具。历史界面展示最近 100 轮、最近 50 个会话，完整上下文仍保留在任务数据库中。
